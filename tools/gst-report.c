...
gboolean     show_memory = FALSE;
gboolean     show_types  = FALSE;
gboolean     hierarchy   = FALSE;
gboolean     nested_time = FALSE;
gboolean     dot         = FALSE;
gboolean     simple_pads = FALSE;
gboolean     dur_only    = FALSE;
gboolean     mu          = FALSE;
...
bind_src_pad (gpointer key, gpointer value, gpointer user_data)
{
  ...
  if (pad->direction == GST_PAD_SRC) {
    ...
    g_print ("en%p_pad_%p -> en%p_pad_%p [style=\"invis\"];\n", sink_pad->parent_element, sink_pad->identifier,
             pad->parent_element, pad->identifier);
  }
}
...
render_headstone (GstGraveyard *graveyard, GstElementHeadstone *element, gsize max_length, gsize max_type_name_length)
{
  ...
  if (dot) {
    ...
    g_print ("label=<<TABLE BORDER=\"0\" CELLBORDER=\"0\" CELLSPACING=\"0\" "
             ...
             "</TR>"
             "</TABLE>>;\n",
             element->type_name->str, /*element, */ element->name->str, time_string,
             total_cpu_time * 100.f / graveyard->total_cpu_time,
             (nested_time ? gst_element_headstone_get_nested_load (element) : element->cpu_load) * 100.0f);
  ...
  if (!dot) {
    ...
    g_print (" %5.1f  %5.1f  %8s",
             (nested_time ? gst_element_headstone_get_nested_load (element) : element->cpu_load) * 100.f,
...
render_connection (gpointer key, gpointer value, gpointer user_data)
{
  ...
  if (pad->direction == GST_PAD_SRC) {
    ...
    g_print ("en%p_pad_%p -> en%p_pad_%p [label=\"%s\"];\n", element->identifier, pad->identifier, pad->peer_element,
             pad->peer, memory_sent_size_string);
    g_free (memory_sent_size_string);
  }
}
